...
function App() {
  ...
  const fetchThumbnails = async (category) => {
    try {
      setIsLoading(true);
      const response = await fetch(`${config.API_URL}/thumbnails/${category}`);
      if (!response.ok) {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      const newDesigns = await Promise.all(files.map(async (file) => {
        ...
        const response = await fetch('${config.API_URL}/get-upload-url', {
          method: 'POST',
          headers: {
  ...
  const generateMockups = async (templates, designs, category) => {
    try {
      const response = await fetch('${config.API_URL}/generate-mockup', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
